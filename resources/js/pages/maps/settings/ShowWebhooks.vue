...
import { AtSign, Bell, Pencil, Rocket, Route, Swords, Trash2, Webhook } from 'lucide-vue-next';
...
type TTriggerTypeMeta = { value: TMapWebhookType; label: string; description: string; icon: Component };
...
const triggerMeta = (type: TMapWebhookType): TTriggerTypeMeta => triggerTypes.find((trigger) => trigger.value === type) ?? triggerTypes[0];
        ...
        <div class="space-y-6">
            ...
            <Card>
                ...
                <CardContent>
                    ...
                    <ul v-else class="divide-y divide-border rounded-lg border">
                        <li v-for="alert in alerts" :key="alert.id" class="flex items-start gap-3 px-3 py-2.5">
                            <div class="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-md border bg-muted/50">
                                <component :is="triggerMeta(alert.type).icon" class="size-4 text-muted-foreground" />
                            </div>
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap items-center gap-2">
                                    <span class="truncate font-medium">
                                        <template v-if="alert.type === 'killmail'">
                                            Kills within {{ alert.max_jumps }} jump{{ alert.max_jumps === 1 ? '' : 's' }} of the chain
                                        </template>
                                        <template v-else-if="alert.type === 'jump_range'">
                                            Exits within {{ maxRangeLy(alert.ship_type ?? 'dreadnought', alert.jdc_level ?? 5).toFixed(1) }} ly of
                                            {{ systemName(alert.target_solarsystem_id) }}
                                        </template>
                                        <template v-else>
                                            {{ systemName(alert.target_solarsystem_id) }} within {{ alert.max_jumps }} gate jump{{
                                                alert.max_jumps === 1 ? '' : 's'
                                            }}
                                        </template>
                                    </span>
                                ...
                                <div class="text-sm text-muted-foreground">
                                    ...
                                    <template v-if="roleName(alert.map_webhook_role_id)">
            ...
            <DialogContent class="max-h-[85vh] overflow-y-auto sm:max-w-3xl">
                ...
                <div class="space-y-6 py-1">
                    <section class="space-y-3">
                        <h4 class="text-xs font-semibold tracking-wider text-muted-foreground uppercase">Trigger</h4>
                        <div class="grid gap-2" role="radiogroup" aria-label="Trigger type">
                            <button
                                v-for="trigger in triggerTypes"
                                :key="trigger.value"
                                type="button"
                                role="radio"
                                :aria-checked="alertForm.type === trigger.value"
                                ...
                                "
                                @click="alertForm.type = trigger.value"
                            >
                                ...
                                <span class="min-w-0">
                                    ...
                                    <span class="block text-xs text-muted-foreground">{{ trigger.description }}</span>
                                </span>
                            </button>
                        </div>
                    </section>
                    ...
                    <section class="space-y-3">
                        <h4 class="text-xs font-semibold tracking-wider text-muted-foreground uppercase">
                            {{ isKillmail ? 'Kill matching' : isJumpRange ? 'Jump range' : 'Range' }}
                        </h4>
                    ...
                    <section class="space-y-3">
                        <h4 class="text-xs font-semibold tracking-wider text-muted-foreground uppercase">Delivery</h4>
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div class="space-y-1.5">
                                <Label>Webhook</Label>
                                <Select
